...
def sync_transactions(
    access_token: str,
    date_from: str,  # kept for signature parity; /sync uses cursor not window
    date_to: str,
    *,
    backoff_fn: Callable[[int], float] | None = None,
) -> Iterable[dict[str, Any]]:
    """Sync transactions via /transactions/sync with pagination + bounded retry (ADR §Connector)."""
...
def land_raw(item_id: str, transactions: list[dict[str, Any]]) -> int:
    """Insert one row per transaction into raw_transactions with canonical JSON (ADR §Raw landing)."""
