...
def render_balance_sheet(period: str, engine: "Engine") -> str:
    ...
    query = text("""
        SELECT 
            ...
            COALESCE(SUM(
                CASE 
                    WHEN jl.side = 'debit' AND a.type IN ('asset', 'expense') THEN jl.amount
                    WHEN jl.side = 'credit' AND a.type IN ('liability', 'equity', 'revenue') THEN jl.amount
                    WHEN jl.side = 'debit' AND a.type IN ('liability', 'equity', 'revenue') THEN -jl.amount
                    WHEN jl.side = 'credit' AND a.type IN ('asset', 'expense') THEN -jl.amount
                    ELSE 0
                END
            ), 0) as balance
        FROM accounts a
        ...
        WHERE je.txn_date IS NULL OR (je.txn_date >= :start_date AND je.txn_date <= :end_date)
...
def render_cash_flow(period: str, engine: "Engine") -> str:
    ...
    investing_activities = []  # Empty for now
    financing_activities = []  # Empty for now
    ...
    for txn_id, txn_data in sorted(transactions.items()):
    ...
    net_operating = sum(
        Decimal(item["amount"]) for item in operating_activities
    )
...
def write_pdf(html: str, out_path: Path) -> Path:
    ...
    """
    msg = "write_pdf not implemented yet"
