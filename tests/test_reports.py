...
def db_engine() -> Engine:
    ...
    with engine.begin() as conn:
        ...
        seed_sql = seed_sql.replace("DELETE FROM", "DELETE FROM").replace("gen_random_uuid()", "'test-uuid'")
        ...
        conn.execute(text("""
            INSERT INTO accounts (id, code, name, type, is_cash) VALUES
                ...
                ('acc2', 'Expenses:Dining:Restaurants', 'Restaurant Expenses', 'expense', 0)
        """))
        
        # Insert test data
        conn.execute(text("""
            INSERT INTO journal_entries (id, item_id, txn_id, txn_date, description, currency, source_hash, transform_version) VALUES
                ('entry1', 'test_item', 'test_txn_001', '2024-01-15', 'Test Restaurant Purchase', 'USD', 'abc123def456', 1)
        """))
...
def test_balance_sheet_html_matches_golden_snapshot(db_engine: Engine) -> None:
    ...
    # Expected hash will be set once we have golden file
    expected_hash = "PLACEHOLDER_HASH_WILL_BE_SET_IN_GREEN_PHASE"
    ...
    assert len(amounts) > 0, "HTML must contain properly formatted amounts (2 decimal places)"
...
def test_pdf_is_emitted_but_not_snapshotted(db_engine: Engine) -> None:
    ...
    """
    from etl.reports.render import render_balance_sheet, write_pdf
...
def test_running_report_twice_produces_identical_html_hash(db_engine: Engine) -> None:
    ...
    assert len(amounts1) > 0 and len(amounts2) > 0, "Both runs must contain properly formatted amounts"
