...
def test_reconcile_success_zero_variance_and_balanced() -> None:
    ...
    with engine.begin() as conn:
        ...
        assert result["checks"]["cash_variance"]["variance"] == pytest.approx(0.00, abs=1e-2)
...
def test_reconcile_fails_on_unbalanced_entries() -> None:
    ...
    with engine.begin() as conn:
        ...
        assert "unbalanced-001" in result["checks"]["entry_balance"]["unbalanced_entries"]
...
def test_reconcile_fails_on_cash_ledger_variance() -> None:
    ...
    with engine.begin() as conn:
        ...
        assert result["checks"]["cash_variance"]["variance"] == pytest.approx(50.00, abs=1e-2)
        assert result["checks"]["cash_variance"]["tolerance"] == pytest.approx(0.01, abs=1e-2)
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("""
            INSERT INTO journal_entries (id, txn_id, txn_date, description, currency,
                                        source_hash, transform_version)
            VALUES 
                (1, 'missing-lineage-001', '2024-01-15', 'No lineage', 'USD', NULL, NULL),
                (2, 'partial-lineage-001', '2024-01-16', 'Partial lineage', 'USD', 'hash', NULL)
        """))
