...
class TestExtract:
    def test_pagination_across_multiple_pages(
        self, monkeypatch: Any, database_url: str
    ) -> None:
        ...
        with respx.mock:
            ...
            count = land_raw("test_item_id", transactions)
            assert count == 2

            # Verify they're in the database
            with connect(database_url) as conn, conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) FROM raw_transactions WHERE item_id = %s", ("test_item_id",))
    ...
    def test_retries_on_429_and_5xx_then_success(self, monkeypatch: Any) -> None:
        ...
        def mock_request(request: httpx.Request) -> httpx.Response:
            nonlocal call_count
            call_count += 1
            if call_count == 1:
    ...
    def test_raw_json_persisted_exactly_with_fetched_at(
        ...
        with connect(database_url) as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT txn_id, as_json, fetched_at FROM raw_transactions WHERE item_id = %s",
                ("test_item_id",),
            )
            row = cur.fetchone()
